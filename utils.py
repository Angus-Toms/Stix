...
def res_count(mcms: List[int]) -> int:
    """
    Find number of res properties based on a list of MCMs 
    """
    tot = 0
    for mcm in mcms:
        if mcm in const.res_mcm:
            tot += 1

    return tot


def non_res_count(mcms: List[int]) -> int:
    """
    Find number of non-res properties based on a list of MCMs
    """
    tot = 0
    for mcm in mcms:
        if mcm in const.non_res_mcm:
            tot += 1

    return tot


def get_res_mcm(mcm: int) -> str:
    """
    Return sector type of given mcm 
    """
    return const.res_mcm[mcm]


def get_non_res_mcm(mcm: int) -> str:
    """
    Return sector type of given mcm  
    """
    return const.non_res_mcm[mcm]


def is_res_mcm(mcm: str) -> bool:
    """
    Check whether an MCM code is valid for residential prop
    """
    return int(mcm) in const.res_mcm


def is_non_res_mcm(mcm: str) -> bool:
    """
    Check whether an MCM code is valid for non-residential prop
    """
    return int(mcm) in const.non_res_mcm
